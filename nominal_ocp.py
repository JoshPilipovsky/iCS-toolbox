...
N_ocp  = 120                      # number of shooting intervals
...
class NominalOCP:
    def __init__(self,
        ...
        self.U     = self.opti.variable(2, N_ocp)         # controls  (σ,α)
        ...
        sigma_init = np.linspace(sigma0, sigma1, N_ocp)
        alpha_init = np.full(N_ocp, alpha0)
        ...
        for i, val in enumerate(xT_target):
            if val is not None:
                self.opti.subject_to(self.X[i,-1] == nd_state([val if j==i else 0
                                                               for j in range(6)])[i])
        ...
        self.opti.subject_to(self.opti.bounded(-np.pi/2, self.U[1,:], np.pi/2))  # α ∈ [−90°,+90°]
        self.opti.subject_to(self.opti.bounded(-np.pi/2, self.U[1,:], np.pi/2))
...
if __name__ == "__main__":
    ...
    t_sim, X_sim = simulate_nominal(x0_mean, sigma_init, alpha_init, Tf_guess, N)
    ...
    labels = ["Altitude [kft]", "θ [deg]", "φ [deg]", "Speed [kft/s]", "ψ [deg]", "γ [deg]"]
