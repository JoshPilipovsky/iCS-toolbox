...
class HypersonicVehicle(NonlinearSDE):
    ...
    def aero_forces(self, h: float, v: float,
    ...
    def f(self, x, u, t):
        ...
        rdot = v * ca.sin(γ)
        θdot = v * ca.cos(γ) * ca.cos(ψ) / (r * ca.cos(φ))
        φdot = v * ca.cos(γ) * ca.sin(ψ) / r
        vdot = (a_s - g_r * ca.sin(γ) + (OMEGA_E**2) * r * ca.cos(φ) *
                (ca.sin(γ) * ca.cos(φ) - ca.cos(γ) * ca.sin(φ) * ca.sin(ψ)))
        ψdot = (a_w / (v * ca.cos(γ)) -
                (v / r) * ca.cos(γ) * ca.tan(φ) * ca.cos(ψ) -
                (OMEGA_E**2 * r /
                 (v * ca.cos(γ))) * (ca.sin(φ) * ca.cos(φ) * ca.cos(ψ)) +
    ...
    def f_np(self, x, u, t):
        ...
        rdot = v * np.sin(γ)
        θdot = v * np.cos(γ) * np.cos(ψ) / (r * np.cos(φ))
        φdot = v * np.cos(γ) * np.sin(ψ) / r
        vdot = (a_s - g_r * np.sin(γ) + (OMEGA_E**2) * r * np.cos(φ) *
                (np.sin(γ) * np.cos(φ) - np.cos(γ) * np.sin(φ) * np.sin(ψ)))
        ψdot = (a_w / (v * np.cos(γ)) -
                (v / r) * np.cos(γ) * np.tan(φ) * np.cos(ψ) -
                (OMEGA_E**2 * r /
                 (v * np.cos(γ))) * (np.sin(φ) * np.cos(φ) * np.cos(ψ)) +
                2 * OMEGA_E * (np.tan(γ) * np.cos(φ) * np.sin(ψ) - np.sin(φ)))

        γdot = (a_n / v - g_r * np.cos(γ) / v + v * np.cos(γ) / r +
